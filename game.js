/*
   ______       ____       _________     _______    ________       ______      _______      _______         _____
 /    __   \     /  __   \   |___     ___|   |    ____|   |   ___    \     /   ___   \   |__     __|    |    ___   \      /  __   \
|   /    \   |   |  |    \_\        |   |          |   |          |  |     \   \    |  |      |  |       |   |        |   |     \   \    |  |    \_\
|  |      |  |   \  \____         |   |          |   |__       |  |___/   /    |  |      |  |       |   |        |   |     |   |    \  \____
|  |___ |  |   \____   \        |   |          |   __|       |   ___   /     |  |      |  |       |   |        |   |      |   |    \___    \ 
|   ____   |    __    \  |       |   |          |  |           |   |   \   \     |  |      |  |       |   |        |   |      |   |    __    \   |
|  |       |  |  \  \__/  /        |   |          |  |_____   |   |   \   \     |  |___/  |    __|   |___   |   |___/   /     \  \__/  / 
|_|       |_|    \____ /         |__|         |_______|  |__|   \__\     \______/    |________|  |_______/       \____ / 

*/

// Copyright (c) 2014 Doug Bruce
//
...
Text = {
  ...
  renderText: function(text, size, x, y) {
    ...
    this.context.scale(pixels, -2 * pixels);
    this.context.beginPath();
	this.context.fontStyle = "arial";
	this.context.fillStyle = "yellow";
	this.context.lineWidth = 3.0;	
...
Game = {
  ...
  FSM: {
	...
	Text.context.fillStyle = "white";
      Text.context.font="42px Arial";
    Text.context.fillText("Press Space to start", Game.canvasWidth/2-210, Game.canvasHeight/2);
    ...
    new_level: function () {
      ...
      if (Date.now() - this.timer > 1000) {
        this.timer = null;
        Game.totalAsteroids++;
        if (Game.totalAsteroids > 4) Game.totalAsteroids = 6;
        Game.spawnAsteroids();
        Game.lives +=1;
		Game.level +=1;
        this.state = 'run';
    ...
    Text.context.fillText(message, Game.canvasWidth/2-150, Game.canvasHeight/2 + 10);
     var score_text = 'Score: '+Game.score;
	var level_text = 'Level:  ' +Game.level; 
	Text.context.fillStyle = "white";
      Text.context.font="22px Arial";
    Text.context.fillText(score_text, Game.canvasWidth - 170, 30);
	Text.context.fillText(level_text, Game.canvasWidth - 170, 50);
	  if (this.timer == null) {
        this.timer = Date.now();
      }
...
$(function () {
  ...
  var mainLoop = function () {
    ...
    // score
	var real_lives = Game.lives + 1;
    var score_text = 'Score: '+Game.score;
	var level_text = 'Level:  ' +Game.level; 
	var g_lives = 'Lives: ' + real_lives;
	Text.context.fillStyle = "white";
      Text.context.font="22px Arial";
    Text.context.fillText(score_text, Game.canvasWidth - 170, 30);
	Text.context.fillText(level_text, Game.canvasWidth - 170, 50);
	Text.context.fillText(g_lives, Game.canvasWidth - 170, 70);	
    
    //Text.renderText(score_text, 22, Game.canvasWidth - 24 * score_text.length, 40);

    // extra dudes
    for (i = 0; i < real_lives; i++) {
      context.save();
      extraDude.x = Game.canvasWidth - (15 * (i+5));
      extraDude.y = 90       ;
	...
	Text.context.fillStyle = "white";
      Text.context.font="42px Arial";
    Text.context.fillText("PAUSED",  Game.canvasWidth/2-100, Game.canvasHeight/2);
